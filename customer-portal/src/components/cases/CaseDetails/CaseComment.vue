...
const { comment, caseId } = defineProps<{
	comment: CommentItem
	caseId: number
}>()
...
async function updateComment() {
	if (!caseId || !newComment.value?.trim()) return

	updating.value = true

	try {
		const response = await Api.cases.updateComment({
			caseId,
...
async function deleteComment() {
	...
	try {
		const response = await Api.cases.deleteComment(comment.id)
