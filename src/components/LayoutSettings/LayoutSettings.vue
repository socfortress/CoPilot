<template>
	<div class="layout-settings flex items-center justify-center shadow-xl" :class="{ open }">
		<XyzTransition mode="out-in">
			...
			<div class="ls-form flex flex-col" v-if="open" key="form" xyz="right-100%">
				<div class="ls-header flex items-center justify-between">
					...
					<div class="ls-icon flex items-center">
						<n-icon @click="open = false" :size="20">
							<CloseIcon />
						</n-icon>
					</div>
				</div>
					...
					<div class="ls-section ls-color-selection">
						...
						<div class="palette flex justify-between">
							<n-button text v-for="color of palette" :key="color.light" @click="setPrimary(color)">
								<template #icon>
									<n-icon :color="theme === ThemeEnum.Dark ? color.dark : color.light" :size="24">
										<ColorIcon />
									</n-icon>
								</template>
							</n-button>
						...
						<div class="flex items-center gap-2">
							<div class="basis-1/2">
								...
								>
									<template #icon>
										<n-icon>
											<LigthIcon v-if="theme === ThemeEnum.Light" />
											<LigthOutlineIcon v-else />
										</n-icon>
									</template>
									Light
								</n-button>
							...
							<div class="basis-1/2">
								...
								>
									<template #icon>
										<n-icon>
											<DarkIcon v-if="theme === ThemeEnum.Dark" />
											<DarkOutlineIcon v-else />
										</n-icon>
									</template>
									Dark
								</n-button>
...
import { NIcon, NColorPicker, NButton, NSelect, useOsTheme, NScrollbar, NSwitch } from "naive-ui"
import { useThemeStore } from "@/stores/theme"
import SettingsIcon from "@vicons/carbon/SettingsAdjust"
import CloseIcon from "@vicons/carbon/Close"
import LigthIcon from "@vicons/ionicons5/Sunny"
import DarkIcon from "@vicons/ionicons5/Moon"
import LigthOutlineIcon from "@vicons/ionicons5/SunnyOutline"
import DarkOutlineIcon from "@vicons/ionicons5/MoonOutline"
import ColorIcon from "@vicons/carbon/CircleSolid"
