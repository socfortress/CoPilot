...
_RULES = [
    ...
    ("T1490", "Resize shadow storage to evict copies", MALICIOUS, re.compile(r"vssadmin(\.exe)?\s+resize\s+shadowstorage", re.IGNORECASE)),
    (
        "T1490",
        "Delete backup catalog / system state",
        MALICIOUS,
        re.compile(r"wbadmin(\.exe)?\s+delete\s+(catalog|systemstatebackup|backup)", re.IGNORECASE),
    ),
    (
        "T1490",
        "Disable Windows recovery / boot repair",
        SUSPICIOUS,
    ...
    (
        "T1003",
        "Mimikatz / LSASS credential access",
        MALICIOUS,
    ...
    (
        ...
        MALICIOUS,
        re.compile(
            r"(comsvcs\.dll[^\n]{0,40}minidump" r"|procdump[^\n]{0,40}lsass" r"|rundll32[^\n]{0,60}comsvcs[^\n]{0,40}full)",
            re.IGNORECASE,
        ),
    ),
    ...
    (
        "T1562.001",
        "Disable AMSI (script scanning bypass)",
        SUSPICIOUS,
    ...
    (
        ...
        SUSPICIOUS,
        re.compile(r"(netsh\s+advfirewall\s+set\s+\w+\s+state\s+off" r"|set-netfirewallprofile[^\n]{0,40}-enabled\s+false)", re.IGNORECASE),
    ),
    (
        "T1562.001",
    ...
    (
        ...
        re.compile(
            ...
            r"|(iex|invoke-expression)\s*\(?\s*\(?\s*(new-object|iwr|curl|wget|invoke-webrequest|invoke-restmethod))",
            re.IGNORECASE,
        ),
    ),
    (
        "T1105",
        "Ingress tool transfer via LOLBin",
        SUSPICIOUS,
        re.compile(
            r"(certutil(\.exe)?[^\n]{0,60}-urlcache[^\n]{0,60}(-split\s+)?-f"
            r"|bitsadmin(\.exe)?[^\n]{0,60}/transfer"
    ...
    (
        "T1218.005",
        "mshta remote/inline script execution",
        SUSPICIOUS,
    ...
    (
        "T1218.011",
        "rundll32 inline script execution",
        SUSPICIOUS,
    ...
    (
        ...
        SUSPICIOUS,
        re.compile(
            r"((reg(\.exe)?\s+add|new-itemproperty|set-itemproperty)[^\n]{0,120}" r"(currentversion\\run|\\runonce))",
            re.IGNORECASE,
        ),
    ),
    (
    ...
    (
        "T1548.002",
        "Known UAC-bypass helper",
        SUSPICIOUS,
        re.compile(
            r"(fodhelper|eventvwr|computerdefaults|sdclt)(\.exe)?[^\n]{0,40}(ms-settings|delegateexecute|shell\\open)",
            re.IGNORECASE,
        ),
    ),
    ...
    (
        "T1055",
        "In-memory shellcode injection primitives",
        MALICIOUS,
        re.compile(
            r"virtualalloc\w*[^\n]{0,200}(writeprocessmemory|memcpy|copy)[^\n]{0,200}(createremotethread|createthread|queueuserapc)",
            re.IGNORECASE | re.DOTALL,
        ),
    ),
    (
        "T1620",
        "Reflective .NET assembly load from bytes",
        SUSPICIOUS,
    ...
    ("T1140", "Decode staged payload via certutil", SUSPICIOUS, re.compile(r"certutil(\.exe)?[^\n]{0,40}-decode(hex)?", re.IGNORECASE)),
    ...
    (
        ...
        SUSPICIOUS,
        re.compile(
            r"(net(\.exe)?\s+localgroup\s+administrators[^\n]{0,40}/add" r"|add-localgroupmember[^\n]{0,40}administrators)",
            re.IGNORECASE,
        ),
    ),
    ...
    (
        "T1021.002",
        "Remote execution (PsExec/WinRS/Invoke-Command)",
        SUSPICIOUS,
    ...
    (
        "T1059.005",
        "WScript.Shell / URLDownloadToFile execution",
        SUSPICIOUS,
        re.compile(
            r"(urldownloadtofile"
            r"|createobject\(\s*['\"]wscript\.shell['\"]\s*\)[^\n]{0,120}\.(run|exec)"
            r"|\.(run|exec)\s*\(?\s*['\"]?[^\n'\"]{0,40}(powershell|cmd(\.exe)?\s|mshta|https?://))",
            re.IGNORECASE,
        ),
    ),
    (
        "T1059.005",
        "VBA Shell() to interpreter",
        SUSPICIOUS,
