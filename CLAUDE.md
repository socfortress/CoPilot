...
- **Sidebar menu (#1152)** — `app-layouts/common/Navbar/items.tsx` plus one file per section in `items/` (Incidents, Investigate, Respond, Detections, Exposure, Endpoints, Reports, Platform), ordered the way SOC work happens. Three rules, all pinned by `Navbar/__tests__/items.spec.ts`: **a leaf's key is its route name** (Navbar highlights the current page by route name; a section key must never equal a route name); the one entry that opens a panel rather than a page (Active Response) uses the exported `ACTIVE_RESPONSE_PANEL_KEY` constant handled in `Navbar.vue` (Stack Provisioning used to be the second one; it is now the `/stack-provisioning` page, and its `StackProvisioningTabs.vue` is the same component the Overview modal mounts); and **Platform mirrors route access**, so analysts see everything they could open except the admin-only Notifications pages and SSO / Audit Log. Don't make a section admin-only without checking its routes' `meta.roles`: most Platform pages allow analysts. The avatar menu holds only Profile, docs, contact and logout. Settings pages belong in Platform.
