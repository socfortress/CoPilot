...
### Tier 2 detonation (P2) — CAPEv2 stood up + wired, live-tested for Linux + Windows + interactive VNC console
...
**CoPilot wiring (`.env`):** `SANDBOX_BACKEND=remote`, `CAPE_API_URL=http://<CAPE_HOST>:8000/apiv2`, `CAPE_API_TOKEN=…`. Restart backend → `get_backend()` returns `CapeBackend`, `sandbox_available` True, Detonation/Network/Live-Session tabs activate. **`env.read_env()` from `backend/` does NOT auto-find the repo-root `.env`** — standalone probe scripts must pass the absolute path; the running app loads it explicitly at startup so it's fine there.
...
**Live Session (Guacamole VNC console) — DONE, proven live.** The "Live Session" tab opens CAPE's **VNC Console** (`/guac/direct/vnc/<vm>/`, the `direct_vnc_vm` view), which boots the guest on demand, streams its desktop over guacd, and **defaults the guest route to `none` (no internet)** — safest while isolation is pending. Chosen over the per-task `/guac/<task_id>/<b64 session_data>/` watch mode (that URL regex `[\w=]+` rejects the `-` in urlsafe-base64, needs a trailing slash, and only works while a task runs). Setup on the CAPE box (all one-time):
- `apt install guacd` (Guacamole 1.3.0) → `systemctl enable --now guacd`, listens `127.0.0.1:4822`.
- `conf/web.conf [guacamole]`: `enabled = yes` **and** `vnc_console_enabled = yes` (the real key is ~line 281, NOT the first `enabled=no` right under the header — and there was a **duplicate `vnc_console_enabled` that DuplicateOptionError-crashed cape-web**; must be exactly one). `mkdir -p storage/guacrecordings` (chown cape).
- **cape-web MUST run under ASGI/daphne, not `runserver_plus` (WSGI)** — the guac pixel stream is a Channels websocket at `/guac/websocket-tunnel/<session>`; under WSGI it 404s and the client shows `guac error 519`. Fix = systemd drop-in `/etc/systemd/system/cape-web.service.d/override.conf` setting `ExecStart=…/poetry run python manage.py runserver 0.0.0.0:8000 --noreload` (with `daphne` first in `INSTALLED_APPS`, plain `runserver` becomes daphne's ASGI runserver → serves http/apiv2 + ws + static-in-DEBUG). `channels`/`daphne` are already in cape's poetry venv; `DEBUG=True`. Verify the startup log says `daphne.server:Listening on TCP address 0.0.0.0:8000`.
- Guest VNC is `autoport` on `127.0.0.1` (display :0 = 5900); the consumer's `_get_vnc_port` reads it from the domain XML — no `labels_and_ports` needed in this CAPE build. Proven: full interactive Win11 desktop streamed in-browser, route None.
- **CoPilot side:** `.env` `CAPE_GUAC_BASE=http://<CAPE_HOST>:8000`, `CAPE_GUAC_VM_LABEL=capewin`. `cape.interactive_url()` returns `{CAPE_GUAC_BASE}/guac/direct/vnc/{CAPE_GUAC_VM_LABEL}/`; `routes.interactive` no longer requires a prior detonation (console is per-VM). **LiveSessionTab.vue** opens it in a **new tab** (real anchor → no popup-block; a full VM desktop deserves a full window) with an embedded-iframe preview fallback. Backend runs from the **`CoPilot/.venv`** venv (`.venv/Scripts/python.exe -m uvicorn copilot:app --port 5000`), NOT bare Python311 (which lacks `dotenv`/`sqlmodel`); `copilot.py` `load_dotenv`s the repo-root `.env`, so a restart picks up new vars.
...
**Still to do on Tier 2:** the **capewin snapshot** (libvirt upgrade → external memory snapshot, needs Ubuntu-24.04 host); **network isolation + firewall on the CAPE box** (`routing.conf route=none` + host firewall limiting `:8000` to the CoPilot IP and no guest egress) — **mandatory before any REAL malware; benign samples only until then**. Note `:8000` is currently **publicly reachable** (guac console + apiv2) — the firewall work must lock it down. CAPE false-positive tuning (benign PowerShell scores malscore 10). Graylog ship + detection rules (P4). git commit (all still uncommitted on branch 974).
