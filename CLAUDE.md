...
### Detection Catalog (view layer over `rules_cache`)

The Detection Catalog (top-level nav, `/detection-catalog`) is a discovery surface — *not* its own data store. It's a pure view over two existing in-memory caches:

- **`rules_cache`** — the CoPilot Searches rule corpus (each rule carries `analytic_story`, `product`, `data_source`, `mitre_attack_id`, `tactics`, `type`, `references`, …).
- **`mitre_matrix`** — used to resolve `mitre_attack_id` → tactic display names when a rule doesn't declare tactics explicitly.

Consequence: refreshing CoPilot Searches refreshes the catalog. There is no separate fetcher, no separate cache, no migration, and no new table. The aggregation (group by `analytic_story` → derive products / data sources / tactics / counts) happens *fresh on each request* inside `app/integrations/copilot_searches/services/detection_catalog.py`. If you find yourself adding persistence here, stop — the design is intentionally a thin projection so the catalog can never drift from the rules.

Wiring layout (everything stays inside the `copilot_searches` namespace on the backend and a new `detectionCatalog/` namespace on the frontend):

- Backend service: `app/integrations/copilot_searches/services/detection_catalog.py`
- Backend routes (appended): `app/integrations/copilot_searches/routes/copilot_searches.py` — `GET /catalog/stats`, `GET /catalog/stories`, `GET /catalog/stories/{story_name:path}`. The `:path` converter on `story_name` is required — story names contain spaces and punctuation (`3CX Supply Chain Attack`, `AWS IAM Privilege Escalation`, …).
- Frontend view: `src/views/DetectionCatalog.vue` (thin wrapper) → `src/components/detectionCatalog/{DetectionCatalogShell,StoriesIndex,StoryDetail}.vue` + `src/types/detectionCatalog.d.ts` + `src/api/endpoints/detectionCatalog.ts`.
- Three authorized cross-cutting edits: nav entry in `app-layouts/common/Navbar/items.tsx`, route in `router/index.ts`, barrel registration in `api/index.ts`. Nothing else outside the catalog namespace should change for this feature.
