...
- **Nothing is shared between instances.** Each tenant gets its own Graylog index set (`office365-<code>-<slug>`, from `instance_slug()`; the unnamed instance keeps the plain `office365-<code>` prefix), its own stream, and its own Grafana datasource, folder and dashboards. A Graylog stream rule pins one value and an AND-matched stream cannot OR several tenant IDs together, which is why one stream cannot serve two tenants. Deletion therefore removes everything the instance's metadata row names, with no shared-resource special case.
