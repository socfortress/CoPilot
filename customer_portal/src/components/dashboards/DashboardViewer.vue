...
async function fetchPanelData() {
	...
	try {
		const res = await SiemAPI.getPanelData(dashboardId, selectedTimerange.value, abortController.signal)

		if (res.success) {
			dashboardTitle.value = res.template.title
			dashboardDescription.value = res.template.description

			panels.value = res.template.panels.map(p => ({
				panel: p,
				data: res.panels[p.id]
			}))
