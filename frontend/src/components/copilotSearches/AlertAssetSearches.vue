			...
			<div class="rounded-lg border border-dashed p-4">
				...
				<n-form :model="formValue" size="small" label-placement="left" label-width="auto">
					...
					<template v-if="ruleDetail">
						<n-form-item v-for="param in ruleDetail.parameters" :key="param.name" :label="param.name">
							<template #label>
								<div class="flex items-center gap-1">
									<span>{{ param.name }}</span>
									<n-tag v-if="param.required" size="tiny" type="error" :bordered="false">
										required
									</n-tag>
								</div>
							</template>
...
import type { ExecuteSearchResponse, PlatformFilter, RuleDetail, RuleSummary } from "@/types/copilotSearches.d"
