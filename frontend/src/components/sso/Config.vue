<template>
	...
	<n-card v-if="isAdmin" title="Single Sign-On (SSO) Configuration" class="mb-4">
						...
						<div class="text-xs">
							...
							<ol>
								...
								<li>
									...
									header into every request. CoPilot verifies the JWT signature against Cloudflare's
									public JWKS — it is cryptographically impossible to forge without Cloudflare's
									private key.
								</li>
								<li>
...
async function loadSettings() {
	...
	try {
		...
		form.value = {
			...
			azure_enabled: res.data.azure_enabled,
			azure_tenant_id: res.data.azure_tenant_id,
			azure_client_id: res.data.azure_client_id,
			azure_client_secret: null,
			azure_redirect_uri: res.data.azure_redirect_uri,
			google_enabled: res.data.google_enabled,
			google_client_id: res.data.google_client_id,
			google_client_secret: null,
			google_redirect_uri: res.data.google_redirect_uri,
			cf_enabled: res.data.cf_enabled,
			cf_team_domain: res.data.cf_team_domain,
			cf_audience: res.data.cf_audience
		}
	} finally {
		loading.value = false
	}
