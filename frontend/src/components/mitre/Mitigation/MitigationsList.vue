<template>
	<div class="flex flex-col gap-3">
		<div v-for="item of itemsPaginated" :key="item.id">
			<MitigationCard :id="item.id" :entity="item.entity" @loaded="item.entity = $event" />
		</div>
...
const mitigations = ref<{ id: string; entity?: MitreMitigationDetails }[]>([])

const itemsPaginated = computed(() => {
	const from = (currentPage.value - 1) * pageSize.value
	const to = currentPage.value * pageSize.value

	return mitigations.value.slice(from, to)
})

onMounted(() => {
	mitigations.value = list.map(o => ({ id: o, entity: undefined }))
})
</script>
