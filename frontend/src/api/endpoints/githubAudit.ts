...
export default {
	...
	createExclusion(configId: number, exclusion: GitHubAuditExclusionCreate, signal?: AbortSignal) {
		return HttpClient.post<GitHubAuditExclusionResponse>(`${BASE_PATH}/config/${configId}/exclusions`, exclusion, {
			signal
		})
	},
	 ...
	 */
	updateExclusion(exclusionId: number, exclusion: GitHubAuditExclusionUpdate, signal?: AbortSignal) {
		return HttpClient.put<GitHubAuditExclusionResponse>(`${BASE_PATH}/exclusions/${exclusionId}`, exclusion, {
			signal
		})
	},
	...
	createBaseline(configId: number, baseline: GitHubAuditBaselineCreate, signal?: AbortSignal) {
		return HttpClient.post<GitHubAuditBaselineResponse>(`${BASE_PATH}/config/${configId}/baselines`, baseline, {
			signal
		})
	},
