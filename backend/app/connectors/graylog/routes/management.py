...
from app.connectors.graylog.services.management import delete_index, stop_input, start_input, stop_stream, start_stream

from app.connectors.graylog.services.collector import (
    get_index_names, get_input_ids
)

from app.connectors.graylog.services.streams import (
    get_stream_ids
)
...
def verify_index_name(deleted_index_body: DeletedIndexBody) -> DeletedIndexBody:
    ...
    if deleted_index_body.index_name not in managed_index_names:
        raise HTTPException(status_code=400, detail=f"Index name '{deleted_index_body.index_name}' is not managed by Graylog or no longer exists.")
...
def verify_stream_id(stop_stream_body: StopStreamBody) -> StopStreamBody:
    ...
    if stop_stream_body.stream_id not in managed_stream_ids:
        raise HTTPException(status_code=400, detail=f"Stream ID '{stop_stream_body.stream_id}' is not managed by Graylog or no longer exists.")
