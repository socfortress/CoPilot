...
from app.connectors.velociraptor.schema.artifacts import (
    ArtifactsResponse, OSPrefixEnum, OSPrefixModel, CollectArtifactBody, CollectArtifactResponse, RunCommandBody, RunCommandResponse, QuarantineBody, QuarantineResponse
)



from app.connectors.velociraptor.services.artifacts import get_artifacts, run_artifact_collection, run_remote_command, quarantine_host
...
@velociraptor_artifacts_router.get("/{os_prefix}", response_model=ArtifactsResponse, description="Get all artifacts for a specific OS prefix")
...
@velociraptor_artifacts_router.get("/hostname/{hostname}", response_model=ArtifactsResponse, description="Get all artifacts for a specific host's OS prefix")
async def get_all_artifacts_for_hostname(hostname: str) -> ArtifactsResponse:
    ...
    return ArtifactsResponse(success=True, message=f"All available artifacts that can be ran for hostname {hostname} retrieved", artifacts=result.artifacts)
...
async def collect_artifact(collect_artifact_body: CollectArtifactBody) -> CollectArtifactResponse:
    ...
    if collect_artifact_body.artifact_name not in artifact_names:
        raise HTTPException(status_code=400, detail=f"Artifact name {collect_artifact_body.artifact_name} does not apply for hostname {collect_artifact_body.hostname} or does not exist")
...
async def run_command(run_command_body: RunCommandBody) -> RunCommandResponse:
    ...
    if run_command_body.artifact_name not in artifact_names:
        raise HTTPException(status_code=400, detail=f"Artifact name {run_command_body.artifact_name.value} does not apply for hostname {run_command_body.hostname} or does not exist")
...
async def quarantine(quarantine_body: QuarantineBody) -> QuarantineResponse:
    ...
    if quarantine_body.artifact_name not in artifact_names:
        raise HTTPException(status_code=400, detail=f"Artifact name {quarantine_body.artifact_name.value} does not apply for hostname {quarantine_body.hostname} or does not exist")
    # Add the velociraptor_id to the run_command_body object
