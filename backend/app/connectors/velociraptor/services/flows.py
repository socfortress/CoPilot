...
async def get_flows(velociraptor_id: str) -> FlowResponse:
    ...
    try:
        if all_flows["success"]:
            flows = [FlowClientSession(**flow) for flow in all_flows["results"]]
            logger.info(f"flows: {flows}")
            return FlowResponse(success=True, message="All flows retrieved.", results=flows)
        else:
            raise HTTPException(status_code=500, detail=f"Failed to retrieve flows from Velociraptor: {all_flows['message']}")
