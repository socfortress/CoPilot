...
from app.connectors.wazuh_indexer.schema.alerts import (
    AlertsSearchBody, AlertsSearchResponse, HostAlertsSearchBody, HostAlertsSearchResponse, IndexAlertsSearchBody, IndexAlertsSearchResponse, AlertsByHostResponse, AlertsByRuleResponse, AlertsByRulePerHostResponse
)

from app.connectors.wazuh_indexer.services.alerts import get_alerts, get_host_alerts, get_index_alerts, get_alerts_by_host, get_alerts_by_rule, get_alerts_by_rule_per_host
...
def verify_index_name(index_alerts_search_body: IndexAlertsSearchBody) -> IndexAlertsSearchBody:
    ...
    if index_alerts_search_body.index_name not in managed_index_names:
        raise HTTPException(status_code=400, detail=f"Index name '{index_alerts_search_body.index_name}' is not managed by Wazuh Indexer or no longer exists.")
...
async def get_all_alerts_for_index(index_alerts_search_body: IndexAlertsSearchBody = Depends(verify_index_name)) -> IndexAlertsSearchResponse:
...
@wazuh_indexer_alerts_router.post("/rules/hosts/all", response_model=AlertsByRulePerHostResponse, description="Get number of all alerts for all rules per host")
