...
async def get_outdated_agents_wazuh(session: AsyncSession) -> OutdatedWazuhAgentsResponse:
    """
    ...
    """
    try:
        wazuh_manager_result = await session.execute(select(Agents).filter(Agents.agent_id == "000"))
        ...
        outdated_agents_result = await session.execute(
            select(Agents).filter(Agents.agent_id != "000", Agents.wazuh_agent_version != wazuh_manager.wazuh_agent_version),
        )
...
async def get_outdated_agents_velociraptor(session: AsyncSession) -> OutdatedVelociraptorAgentsResponse:
    """
    ...
    try:
        ...
        server_version = await velociraptor_service._get_server_version(vql_server_version)
