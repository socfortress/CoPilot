...
async def add_copilot_alert_id(index_data: CreateAlertRequest, alert_id: int):
    ...
    try:
        ...
        logger.error(f"Failed to add CoPilot alert ID {alert_id} to Graylog event {index_data.alert_id} in index {index_data.index_name}: {e}")
        ...
        try:
            ...
            es_client.update(index=index_data.index_name, id=index_data.alert_id, body=body)
            logger.info(f"Added CoPilot alert ID {alert_id} to Graylog event {index_data.alert_id} in index {index_data.index_name} after removing read-only block")
