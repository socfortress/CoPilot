...
from app.healthchecks.agents.schema.agents import AgentHealthCheckResponse, AgentModel, ExtendedAgentModel, TimeCriteriaModel, HostLogsSearchResponse, HostLogsSearchBody, LogsSearchBody, LogsSearchResponse, CollectLogsResponse
from app.connectors.wazuh_indexer.utils.universal import create_wazuh_indexer_client, format_node_allocation, format_indices_stats, format_shards, collect_indices, LogsQueryBuilder
...
def host_logs(search_body: HostLogsSearchBody) -> HostLogsSearchResponse:
    ...
    if total_logs > 0:
        return HostLogsSearchResponse(success=True, healthy=True, message=f"Host is healthy. At least one log was found within the specified time range of {search_body.timerange}")
    else:
        return HostLogsSearchResponse(success=True, healthy=False, message=f"Host is unhealthy. No logs were found within the specified time range of {search_body.timerange}")
