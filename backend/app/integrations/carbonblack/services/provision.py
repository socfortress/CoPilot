...
from app.integrations.carbonblack.schema.provision import CarbonBlackEventStream
...
async def build_index_set_config(
    ...
    return TimeBasedIndexSet(
        title=f"{(await get_customer(customer_code, session)).customer.customer_name} - CARBONBLACK",
        description=f"{customer_code} - CARBONBLACK",
        index_prefix=f"carbonblack-{customer_code}",
...
async def build_event_stream_config(
    ...
    index_set_id: str,
    session: AsyncSession,
) -> CarbonBlackEventStream:
    """
    Builds the configuration for the Huntress event stream.
    ...
    Returns:
        CarbonBlackEventStream: The configured Huntress event stream.
    """
    return CarbonBlackEventStream(
        title=f"{(await get_customer(customer_code, session)).customer.customer_name} - CARBONBLACK",
        description=f"{(await get_customer(customer_code, session)).customer.customer_name} - CARBONBLACK",
...
async def send_event_stream_creation_request(
    event_stream: CarbonBlackEventStream,
...
async def create_grafana_datasource(
    ...
    datasource_payload = GrafanaDatasource(
        ...
        database=f"carbonblack-{customer_code}*",
        basicAuth=True,
...
async def provision_carbonblack(
    provision_carbonblack_request: ProvisionCarbonBlackRequest,
    session: AsyncSession,
) -> ProvisionCarbonBlackResponse:
    logger.info(
        f"Provisioning CarbonBlack integration for customer {provision_carbonblack_request.customer_code}.",
    )
    ...
    # Grafana Deployment
    carbonblack_datasource_uid = (
        await create_grafana_datasource(
            customer_code=provision_carbonblack_request.customer_code,
            session=session,
        )
    ).datasource.uid
    grafana_carbonblack_folder_id = (
    ...
    return ProvisionCarbonBlackResponse(
        success=True,
        message="CarbonBlack integration provisioned successfully.",
    )
...
async def update_customer_integration_table(
    ...
    matches the given customer code and the `integration_service_name` is "Huntress".
