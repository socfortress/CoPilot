...
@integration_nuclei_router.get(
    "/all_reports",
    response_model=NucleiReportsAvailableResponse,
...
@integration_nuclei_router.get(
    "/report/{host}/{report}",
    response_model=NucleiReportCollectionResponse,
    dependencies=[Security(AuthHandler().require_any_scope("admin", "analyst"))],
)
...
@integration_nuclei_router.post(
    "/scan",
...
@integration_nuclei_router.delete(
    "/delete_report/{host}",
    response_model=DeleteNucleiReportResponse,
    dependencies=[Security(AuthHandler().require_any_scope("admin", "analyst"))],
)
