...
@integration_nuclei_router.get("/all_reports", response_model=NucleiReportsAvailableResponse, dependencies=[Security(AuthHandler().require_any_scope("admin", "analyst"))])
async def get_all_reports():
    logger.info("Collecting Nuclei Reports")
    return await get_nuclei_reports_available()


@integration_nuclei_router.get("/report/{host}/{report}", response_model=NucleiReportCollectionResponse, dependencies=[Security(AuthHandler().require_any_scope("admin", "analyst"))])
...
@integration_nuclei_router.post("/scan", response_model=NucleiScanResponse, dependencies=[Security(AuthHandler().require_any_scope("admin", "analyst"))])
async def post_test(
...
@integration_nuclei_router.delete("/delete_report/{host}", response_model=DeleteNucleiReportResponse, dependencies=[Security(AuthHandler().require_any_scope("admin", "analyst"))])
