...
from app.integrations.cato.schema.provision import CatoEventStream
...
async def build_index_set_config(
    ...
    return TimeBasedIndexSet(
        title=f"{(await get_customer(customer_code, session)).customer.customer_name} - Cato",
        description=f"{customer_code} - CATO",
        index_prefix=f"cato_{customer_code}",
...
async def build_event_stream_config(
    ...
    """
    Builds the configuration for the Cato event stream.
    ...
    Returns:
        CatoEventStream: The configured Cato event stream.
    """
    return CatoEventStream(
        title=f"{(await get_customer(customer_code, session)).customer.customer_name} - CATO",
        description=f"{(await get_customer(customer_code, session)).customer.customer_name} - CATO",
...
async def create_grafana_datasource(
    ...
    datasource_payload = GrafanaDatasource(
        ...
        database=f"cato_{customer_code}*",
        basicAuth=True,
...
async def provision_cato(
    provision_cato_request: ProvisionCatoRequest,
    session: AsyncSession,
) -> ProvisionCatoResponse:
    logger.info(
        f"Provisioning Cato integration for customer {provision_cato_request.customer_code}.",
    )

    # Create Index Set
    index_set_id = (
        await create_index_set(
            customer_code=provision_cato_request.customer_code,
            session=session,
        )
    ).data.id
    ...
    await provision_dashboards(
        DashboardProvisionRequest(
            dashboards=[dashboard.name for dashboard in CatoDashboard],
            organizationId=(
                await get_customer_meta(
                    provision_cato_request.customer_code,
                    session,
            ...
            folderId=grafana_Cato_folder_id,
            datasourceUid=Cato_datasource_uid,
        ),
    )
...
async def update_customer_integration_table(
    ...
    matches the given customer code and the `integration_service_name` is "CATO".
