...
from app.integrations.sap_siem.schema.provision import SapSiemEventStream
...
async def send_event_stream_creation_request(
    event_stream: SapSiemEventStream,
...
async def create_grafana_datasource(
    ...
    datasource_payload = GrafanaDatasource(
        ...
        database=f"sap_siem_{customer_code}*",
        basicAuth=True,
        ...
        jsonData={
            ...
            "includeFrozen": False,
            "logLevelField": "level",
            "logMessageField": "params_loginID",
...
async def provision_sap_siem(
    ...
    """
    logger.info(
        f"Provisioning SAP SIEM integration for customer {provision_sap_siem_request.customer_code}.",
    )
    ...
    grafana_sap_siem_folder_id = (
        await create_grafana_folder(
            ...
            folder_title="SAP SIEM",
        )
    ).id
    await provision_dashboards(
        DashboardProvisionRequest(
            dashboards=[dashboard.name for dashboard in SapSiemDashboard],
            organizationId=(
                await get_customer_meta(
                    provision_sap_siem_request.customer_code,
                    session,
            ...
            folderId=grafana_sap_siem_folder_id,
            datasourceUid=sap_siem_datasource_uid,
        ),
    )
...
async def update_customer_integration_table(
    ...
    matches the given customer code and the `integration_service_name` is "SAP SIEM".
