...
#### ! GRAYLOG ! ####
async def build_index_set_config(request: DefenderForEndpointCustomerDetails) -> TimeBasedIndexSet:
    """
    Build the configuration for a time-based index set.

    Args:
        request (DefenderForEndpointCustomerDetails): The request object containing customer information.

    Returns:
        TimeBasedIndexSet: The configured time-based index set.
    """
    return TimeBasedIndexSet(
        title=f"{request.customer_name} - DEFENDER FOR ENDPOINT EVENTS",
        description=f"{request.customer_name} - DEFENDER FOR ENDPOINT EVENTS",
        index_prefix=f"defender-for-endpoint-{request.customer_code}",
...
async def provision_content_pack(customer_details):
    ...
    return await provision_content_pack_network_connector(
        content_pack_request=ProvisionNetworkContentPackRequest(
            content_pack_name="DEFENDER_FOR_ENDPOINT",
...
async def get_stream_and_index_ids(customer_details):
    ...
    """
    stream_id = await get_stream_id_by_stream_name(stream_name=f"{customer_details.customer_name} - DEFENDER FOR ENDPOINT LOGS AND EVENTS")
    ...
    content_pack_stream_id = await get_content_pack_id_by_content_pack_name(
        content_pack_name=f"{customer_details.customer_name}_DEFENDER_FOR_ENDPOINT_STREAM",
    )
    if customer_details.protocal_type == "TCP":
        content_pack_input_id = await get_content_pack_id_by_content_pack_name(
            content_pack_name=f"{customer_details.customer_name}_DEFENDER_FOR_ENDPOINT_INPUT_TCP",
        )
...
async def create_grafana_datasource(
    ...
    datasource_payload = GrafanaDatasource(
        ...
        database=f"defender-for-endpoint-{customer_code}*",
        basicAuth=True,
...
async def provision_defender_for_endpoint(
    customer_details: DefenderForEndpointCustomerDetails,
    keys: ProvisionDefenderForEndpointAuthKeys,
    session: AsyncSession,
) -> ProvisionDefenderForEndpointResponse:
    """
    Provisions a Defender For Endpoint customer by performing the following steps:
    ...
    5. Inserts the customer network connector metadata into the database.
    ...
    logger.info(f"Provisioning Defender For Endpoint for customer {customer_details.customer_name}")
    ...
    grafana_folder = await create_grafana_folder(
        ...
        folder_title="DEFENDER FOR ENDPOINT",
    )
    await provision_dashboards(
        DashboardProvisionRequest(
            dashboards=[dashboard.name for dashboard in DefenderForEndpointDashboard],
            organizationId=(
                await get_customer_meta(
                    customer_details.customer_code,
                    session,
    ...
    await create_customer_data_directory(customer_name=customer_details.customer_name)
    file = await load_and_replace_docker_compose(customer_name=customer_details.customer_name)
    await save_uploaded_file(
        file=file,
        filename=f"{customer_details.customer_name}_docker-compose-defender-for-endpoint.yml",
        customer_name=customer_details.customer_name,
    )
    await load_and_replace_filebeat_cfg(customer_details=customer_details, keys=keys, session=session)
    ...
    return ProvisionDefenderForEndpointResponse(
        message="Defender For Endpoint for customer provisioned successfully",
        success=True,
    )
...
async def create_customer_data_directory(customer_name: str):
    """
    Create a data directory within the customer's directory in the UPLOAD_FOLDER

    This function ensures there's a dedicated data directory for customer-specific files.
    For example, if the customer directory is /opt/CoPilot/data/Customer1,
    this will create /opt/CoPilot/data/Customer1/data.
    ...
    # Create the path to the data directory within the customer's directory
    customer_data_directory = os.path.join(customer_directory, "data")

    # Check if the customer directory exists, if not create it
    if not os.path.exists(customer_directory):
        os.makedirs(customer_directory)

    # Check if the data directory exists, if not create it
    if not os.path.exists(customer_data_directory):
        os.makedirs(customer_data_directory)
        logger.info(f"Created data directory for customer {customer_name}: {customer_data_directory}")
    else:
        logger.info(f"Data directory for customer {customer_name} already exists: {customer_data_directory}")

    return customer_data_directory
...
async def load_and_replace_filebeat_cfg(
    customer_details: DefenderForEndpointCustomerDetails,
    keys: ProvisionDefenderForEndpointAuthKeys,
    session: AsyncSession,
):
    ...
    async with aiofiles.open(os.path.join(customer_upload_folder, "filebeat.yml"), "w") as f:
        await f.write(data)
    return os.path.join(customer_upload_folder, "filebeat.yml")
    ...
    await session.execute(
        ...
        .where(
            and_(
                ...
                CustomerIntegrations.integration_service_name == "DefenderForEndpoint",
            ),
        )
