...
from app.integrations.modules.schema.sap_siem import CollectSapSiemRequest, InvokeSapSiemAnalysis
...
async def post_to_copilot_sap_module_sap_siem_successful_user_login_with_different_ip(data: InvokeSapSiemAnalysis):
    ...
    """
    logger.info(f"Sending POST request to http://copilot-sap-module/sap-siem/same_user_failed_login_from_different_ip with data: {data.dict()}")
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
...
async def post_to_copilot_sap_module_same_user_failed_login_from_different_geo_location(data: InvokeSapSiemAnalysis):
    ...
    Args:
        data (InvokeSapSiemAnalysis): The data to send to the copilot-sap-module Docker container.
    """
    logger.info(f"Sending POST request to http://copilot-sap-module/sap-siem/same_user_failed_login_from_different_geo_location with data: {data.dict()}")
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
...
async def post_to_copilot_sap_module_same_user_successful_login_from_different_geo_location(data: InvokeSapSiemAnalysis):
    ...
    Args:
        data (InvokeSapSiemAnalysis): The data to send to the copilot-sap-module Docker container.
    """
    logger.info(f"Sending POST request to http://copilot-sap-module/sap-siem/same_user_successful_login_from_different_geo_location with data: {data.dict()}")
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
...
async def post_to_copilot_sap_module_brute_force_failed_logins_multiple_ips(data: InvokeSapSiemAnalysis):
    ...
    Args:
        data (InvokeSapSiemAnalysis): The data to send to the copilot-sap-module Docker container.
    """
    logger.info(f"Sending POST request to http://copilot-sap-module/sap-siem/brute_force_failed_logins_multiple_ips with data: {data.dict()}")
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
...
async def post_to_copilot_sap_module_successful_login_after_multiple_failed_logins(data: InvokeSapSiemAnalysis):
    ...
    Args:
        data (InvokeSapSiemAnalysis): The data to send to the copilot-sap-module Docker container.
    """
    logger.info(f"Sending POST request to http://copilot-sap-module/sap-siem/successful_login_after_multiple_failed_logins with data: {data.dict()}")
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
