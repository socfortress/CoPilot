...
async def send_event_stream_creation_request(
    event_stream: HuntressEventStream,
...
async def create_grafana_datasource(
    ...
    datasource_payload = GrafanaDatasource(
        ...
        database=f"huntress_{customer_code}*",
        basicAuth=True,
...
async def provision_huntress(
    provision_huntress_request: ProvisionHuntressRequest,
    session: AsyncSession,
) -> ProvisionHuntressResponse:
    logger.info(
        f"Provisioning Huntress integration for customer {provision_huntress_request.customer_code}.",
    )

    # Create Index Set
    index_set_id = (
        await create_index_set(
            customer_code=provision_huntress_request.customer_code,
            session=session,
        )
    ).data.id
    ...
    # Grafana Deployment
    huntress_datasource_uid = (
        await create_grafana_datasource(
            customer_code=provision_huntress_request.customer_code,
            session=session,
        )
    ).datasource.uid
    grafana_huntress_folder_id = (
    ...
    await provision_dashboards(
        DashboardProvisionRequest(
            ...
            folderId=grafana_huntress_folder_id,
            datasourceUid=huntress_datasource_uid,
        ),
    )
    await create_integration_meta_entry(
        CustomerIntegrationsMetaSchema(
            ...
            grafana_dashboard_folder_id=grafana_huntress_folder_id,
            grafana_datasource_uid=huntress_datasource_uid,
        ),
        session,
    )
