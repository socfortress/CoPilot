...
from app.integrations.monitoring_alert.schema.monitoring_alert import CustomIrisAsset
...
async def construct_alert_source_link(
    ...
    return (
        f"{grafana_url}/explore?left=%5B%22now-6h%22,%22now%22,%22Custom%22,%7B%22refId%22:%22A%22,"
        f"{query_string}"
...
async def check_if_open_alert_exists_in_iris(alert_details: CustomAlertModel, session: AsyncSession) -> list:
    """
    Check if the alert exists in IRIS.

    Args:
        alert_details (CustomAlertModel): The alert details.
...
async def create_alert_details(
    ...
    """
    Create an alert details object from the Custom alert details.

    Args:
        alert_details (CustomAlertModel): The Custom alert details.

    Returns:
        CustomIrisAlertContext: The alert details object.
    """
...
async def create_and_update_alert_in_iris(
    ...
    await fetch_and_validate_data(
        ...
        alert_client.update_alert,
        alert_id,
        {"assets": [dict(CustomIrisAsset(**iris_alert_payload.assets[0].to_dict()))]},
    )
    if ioc_payload:
        await fetch_and_validate_data(
            client,
...
async def analyze_custom_alert(
    ...
    """
    logger.info(f"Analyzing custom alerts: alert_index: {monitoring_alerts.event.alert_index}, alert_id: {monitoring_alerts.event.alert_id}")
    ...
    if iris_alert_id == []:
        ...
        current_assets.append(dict(CustomIrisAsset(**asset_payload.to_dict())))
