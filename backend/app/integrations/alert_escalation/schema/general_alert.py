...
class IrisAsset(BaseModel):
    ...
    asset_ip: str = Field(
        ..., description="IP address of the asset", example="192.168.1.1",
    )
    asset_description: str = Field(
        ..., description="Description of the asset", example="Windows Server",
    )
...
class IrisAlertContext(BaseModel):
    ...
    alert_name: str = Field(
        ..., description="Name of the alert", example="Intrusion Detected",
    )
    alert_level: int = Field(..., description="Severity level of the alert", example=3)
    rule_id: str = Field(
        ..., description="ID of the rule that triggered the alert", example="2001",
    )
    asset_name: str = Field(
        ..., description="Name of the affected asset", example="Server01",
    )
    asset_ip: str = Field(
        ..., description="IP address of the affected asset", example="192.168.1.1",
    )
    ...
    rule_mitre_id: Optional[str] = Field(
        "n/a", description="MITRE ATT&CK ID of the rule", example="T1234",
    )
    rule_mitre_tactic: Optional[str] = Field(
        "n/a", description="MITRE ATT&CK Tactic", example="Execution",
    )
    rule_mitre_technique: Optional[str] = Field(
        "n/a", description="MITRE ATT&CK Technique", example="Scripting",
    )


class IrisAlertPayload(BaseModel):
    alert_title: str = Field(
        ..., description="Title of the alert", example="Intrusion Detected",
    )
    ...
    alert_severity_id: int = Field(
        ..., description="Severity ID of the alert", example=5,
    )
    alert_customer_id: int = Field(
        ..., description="Customer ID related to the alert", example=1,
    )
    ...
    alert_iocs: Optional[List[IrisIoc]] = Field(
        None, description="List of IoCs related to the alert",
    )
