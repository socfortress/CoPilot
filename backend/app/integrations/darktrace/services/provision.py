...
from app.integrations.darktrace.schema.provision import DarktraceEventStream
...
async def build_index_set_config(
    ...
    return TimeBasedIndexSet(
        title=f"{(await get_customer(customer_code, session)).customer.customer_name} - DARKTRACE",
        description=f"{customer_code} - DARKTRACE",
        index_prefix=f"darktrace-{customer_code.lower()}-",
...
async def build_event_stream_config(
    ...
    index_set_id: str,
    session: AsyncSession,
) -> DarktraceEventStream:
    """
    Builds the configuration for the Darktrace event stream.
    ...
    Returns:
        DarktraceEventStream: The configured Darktrace event stream.
    """
    return DarktraceEventStream(
        title=f"{(await get_customer(customer_code, session)).customer.customer_name} - DARKTRACE",
        description=f"{(await get_customer(customer_code, session)).customer.customer_name} - DARKTRACE",
...
async def send_event_stream_creation_request(
    event_stream: DarktraceEventStream,
...
async def create_grafana_datasource(
    ...
    datasource_payload = GrafanaDatasource(
        ...
        database=f"darktrace-{customer_code}*",
        basicAuth=True,
...
async def provision_darktrace(
    provision_darktrace_request: ProvisionDarktraceRequest,
    session: AsyncSession,
) -> ProvisionDarktraceResponse:
    logger.info(
        f"Provisioning Darktrace integration for customer {provision_darktrace_request.customer_code}.",
    )

    # Create Index Set
    index_set_id = (
        await create_index_set(
            customer_code=provision_darktrace_request.customer_code,
            session=session,
        )
    ).data.id
    ...
    # Grafana Deployment
    darktrace_datasource_uid = (
        await create_grafana_datasource(
            customer_code=provision_darktrace_request.customer_code,
            session=session,
        )
    ).datasource.uid
    grafana_darktrace_folder_id = (
        await create_grafana_folder(
            ...
            folder_title="DARKTRACE",
        )
    ).id
    await provision_dashboards(
        DashboardProvisionRequest(
            dashboards=[dashboard.name for dashboard in DarktraceDashboard],
            organizationId=(
                await get_customer_meta(
                    provision_darktrace_request.customer_code,
                    session,
            ...
            folderId=grafana_darktrace_folder_id,
            datasourceUid=darktrace_datasource_uid,
        ),
    )
...
async def update_customer_integration_table(
    ...
    matches the given customer code and the `integration_service_name` is "Darktrace".
